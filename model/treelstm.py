...
class BinaryTreeLSTM(nn.Module):
    ...
    def __init__(self, word_dim, hidden_dim, use_leaf_rnn, intra_attention,
                 temperature, bidirectional, mode):
    ...
    def select_composition(self, old_state, new_state, mask):
        ...
        if self.training:
            if self.mode.startswith('gst-'):
                try:
                    gap = float(self.mode[4:])
                    if gap<0.0: gap = 'p'
                except:
                    gap = 'p'
                select_mask = basic.gst_mover(
                    logits=comp_weights, temperature=self.temperature,
                    mask=mask, gap=gap)
            elif self.mode == 'gumbel':
                select_mask = basic.st_gumbel_softmax(
                    logits=comp_weights, temperature=self.temperature,
                    mask=mask)
            elif self.mode == 'rao_gumbel':
                select_mask = basic.rao_gumbel(
                    logits=comp_weights, temperature=self.temperature,
                    mask=mask)
            else:
                print ('unsupported')
